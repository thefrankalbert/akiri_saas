// ============================================
// Corridors Service — DI factory pattern
// ============================================
...
export function createCorridorsService(supabase: SupabaseClient) {
  return {
    ...
    async getCorridors(): Promise<Corridor[]> {
      ...
      if (error || !data) {
        logger.error('Failed to fetch corridors', { error: error?.message });
        throw new ServiceError(error?.message || 'Failed to fetch corridors', 'INTERNAL');
      }
